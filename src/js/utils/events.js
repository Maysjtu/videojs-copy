...
export function fixEvent(event) {
    ...
    if (!event || !event.isPropagationStopped) {
        ...
        //	element.clientTop 一个数值，代表了元素的上边线框宽度，单位px。
        //todo
...
//https://zjy.name/archives/passive-event-listeners.html
//被动事件监听器（Passive Event Listeners）是 DOM 规范中的一个新特性，
// 它让开发者可以通过排除触摸和滚动事件监听器中阻止默认行为的需求，得到更高的滚动表现。
// 开发者可以给触摸和滚动事件标注使用 { passive: true } 来标注不会调用 preventDefault。
    ...
    if (!data.dispatcher) {
        ...
        data.dispatcher = function(event, hash) {
            ...
            if(handlers) {
                ...
                const handlersCopy = handlers.slice(0); //666
